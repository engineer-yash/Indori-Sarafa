          ...
          >
            ...
            <div className="p-6 md:p-10 flex flex-col gap-4">
              <div className="flex flex-wrap items-center gap-2">
                {item.veg && (
                  <span className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-semibold text-green-700">
                ...
                {item.popular && (
                  <span className="text-[10px] uppercase tracking-wider font-semibold text-[#A52A2A] bg-[#A52A2A]/10 px-2.5 py-1 rounded-full">
                    Popular
                  </span>
                )}
                <span className="text-[10px] uppercase tracking-wider font-semibold text-[#5C2C06] bg-[#5C2C06]/10 px-2.5 py-1 rounded-full">
                  {item.category}
                </span>
              </div>
              <h3 className="font-display text-3xl md:text-4xl font-extrabold text-[#5C2C06] leading-tight">
                {item.name}
              </h3>
              <p className="text-sm md:text-base text-[#5C2C06]/75 leading-relaxed">
                {item.description}
              </p>
              <div className="mt-2 flex items-end gap-2">
                <span className="font-display text-4xl font-extrabold gold-text-gradient">
                  ₹{item.price}
                </span>
                <span className="text-xs text-[#5C2C06]/55 pb-2">per plate</span>
              </div>
              <a
                href="tel:+919545637022"
                className="btn-gold justify-center mt-3"
                data-testid="modal-order-cta"
              >
